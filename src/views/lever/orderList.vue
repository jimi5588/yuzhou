<template>
    <div class="trade-detail">
        <van-nav-bar left-arrow @click-left="onClickLeft">
            <template #left>
                <van-icon name="arrow-left" color="var(--text-color)" size="20" />
                <div class="title-left">
                    <div class="rate-wrap">
                        <span>{{ t('trade_risk_rate') }}: </span>
                        <span class="rate">{{ totalFengxian.toFixed(2) }}%</span>  
                    </div>
                    <span :class="totalProfitPrice < 0 ? 'down' : 'up'">{{ t('trade_total_profit_and_loss') }}:{{ totalProfitPrice.toFixed(5)
                        }}</span>
                </div>

            </template>
            <template #right>
                <van-button type="primary" size="small" class="close-all-btn" @click="closePositionDialog = true">
                    {{ t('trade_close_position_with_one_click') }}
                </van-button>
            </template>
        </van-nav-bar>
...
import { getContractOrderList, setContract, cancelContractOrder, batchCloseContract } from '@/api';
...
const cancelOrder = (item) => {
    showConfirmDialog({
        ...
        .then(() => {
            ...
            showLoadingToast(t('common_loading'))
            cancelContractOrder(params).then(res => {
                showSuccessToast(t('common_success'))
                listRefresh.value = true
            }).catch(err => {
                showFailToast(err.message)
